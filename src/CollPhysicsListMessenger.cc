...
void CollPhysicsListMessenger::SetNewValue(G4UIcommand* command, G4String value)
{
    if (command == _physicsListCmd)
    {
        _physicsList->AddPhysicsList(value);
        return;
    }

    if( command == _gammaCutCmd )
    {
        _physicsList->SetCutForGamma(_gammaCutCmd->GetNewDoubleValue(value));
        return;
    }

    if( command == _electCutCmd )
    {
        _physicsList->SetCutForElectron(_electCutCmd->GetNewDoubleValue(value));
        return;
    }

    if( command == _positCutCmd )
    {
        _physicsList->SetCutForPositron(_positCutCmd->GetNewDoubleValue(value));
        return;
    }

    if( command == _allCutCmd )
    {
        auto cut = _allCutCmd->GetNewDoubleValue(value);
